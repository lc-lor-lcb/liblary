...
## 機能設計（主な機能）
1. 司書ログイン（パスワードまたはバーコードまたはPINコード、二段階認証）
...
7. 履歴管理（貸出・返却・予約履歴）
8. 書籍・蔵書検索（書籍IDまたは蔵書ID・現在の状況・返却予定日表示）
...
- 運用: 毎日閉館後かつ全業務終了後に増分バックアップ、毎月最初の閉館後にフルバックアップ
- 保持: 履歴を過去3年保持
- セキュリティ: TLS、PBKDF2(パスワード)、2016(PINコード)、セッション自動タイムアウトまたはロック15分、2要素認証（メール）
...
## 運用・バックアップ
- 毎日閉館後かつ全業務終了後に増分バックアップ、月一回フルバックアップ（要件通り）
- 監視: アプリヘルス、DB容量、レスポンス時間、エラーレート、バックアップを監視
- 復旧目標: 発生日中に復旧
