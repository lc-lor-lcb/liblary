...
## 業務フロー
### ログイン
1. 利用者が利用者情報を入力し、ログインする
2. 以下のフローへ移行
### 貸し出し
1. 利用者がカウンターに本を持ってくる
2. 利用者が蔵書情報を入力する
3. システムが利用者の返却期限を過ぎていた場合、期限内に返すよう促す旨を自動で表示
4. 貸し出せず、利用者が希望する場合、予約フローへ
5. システムがステータスを貸し出し中に変更
6. システムが最終返却日を表示
### 返却
1. 利用者が蔵書を返却ボックス内に入れる
2. 司書が返却ボックス内の蔵書を蔵書情報を入力する
3. システムが予約されている場合、画面に予約中の旨を表示する
4. システムが蔵書のステータスを未貸し出しに変更する
5. 司書が予約中の場合取り置く
6. 司書が返却完了した蔵書をに置く
### 予約
1. システムが貸し出し予約時に画面に予定返却日を表示
2. システムが予約した蔵書が返却後利用者に通知を送信
3. 予約した利用者が再度、利用者情報と蔵書情報を入力する
4. 司書が取り置いた蔵書を渡す
5. システムが蔵書のステータスを貸し出し中に変更
# 機能要件
## 司書ログイン機能
...
* 著者名、ジャンル、図書名を入力することで書籍IDを新規生成
* 書籍IDから蔵書IDを作成、割り当て可能
* 同じ蔵書でも異なるIDを割り当て、区別する
## 蔵書貸し出し機能
* 既に貸し出されているか自分以外に予約されている場合、貸し出し不可能とする
* 蔵書情報と利用者情報を入力することで貸し出し可能な場合、蔵書の貸し出しが可能
* 1利用につき、5冊まで同時に貸し出し可能
## 蔵書返却機能
...
## 利用者管理機能
* 人名、生年月日、性別、電話番号、メールアドレスを入力することで利用者ＩＤを新規作成、割り当てる
* ログイン中の司書は情報の変更が可能
# 非機能要件
## 可用性
